...
module.exports = {
  ...
  fail(errno, errmsg = '', data = '') {
    ...
    if (helper.isObject(errno)) {
      ...
      if (/^[A-Z_]+$/.test(errno)) {
        const messages = kao.app.validators.messages || {};
        const msg = messages[errno];
        if (kao.isArray(msg)) {
          [errno, errmsg] = msg;
        }
      }
      if (!kao.isNumber(errno)) {
  ...
  post(name, value) {
    if (!this[POST]) {
      ...
      this[POST] = kao.isArray(json) ? Array.from(json) : Object.assign({}, json);
